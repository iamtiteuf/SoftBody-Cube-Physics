...
public:
	...
	void Camera_Controls(bool IsActive)
	{
		if (IsActive)
		{
			...
			else if (glfwGetKey(MainWindow, GLFW_KEY_LEFT_SHIFT) == GLFW_RELEASE)
			{
				speed = 0.1f;
			}
			...
			if (glfwGetMouseButton(MainWindow, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS )
			{
				...
				if (firstClick)
				{
					glfwGetCursorPos(MainWindow, &mouseX2, &mouseY2);
			...
			else if (glfwGetMouseButton(MainWindow, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_RELEASE )
			{
				...
				if (!firstClick)
				{
					...
					glfwSetCursorPos(MainWindow, mouseX2, mouseY2);
				}
